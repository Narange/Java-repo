...
public class FalloutTerminal {
  ...
  public static void main(String[] args){
    FalloutTerminal ft = new FalloutTerminal();
    ft.play();
  }
  
  public void play(){
    
    System.out.println("**Guess the password (ALL CAPS)**");
    System.out.println("Likeness is number of matching letters in correct position.**");
    System.out.println("ROBCO INDUSTRIES (TM) TERMALINK PROTOCOL");
    System.out.println("ENTER PASSWORD NOW\n");
    
    //get the 10 random words
    String[] words = get10Words();
    //get a random word from the 10 and make it PASSWORD
    PASSWORD = words[random.nextInt(10)];
    
    int likeness = -1;
    printChoices(words);
    
    //main game loop
    while (!input.equals("ENDGAME") && (attemptsRemaining > 0)){
      
      //take user input
      input = scanner.nextLine();
      //check input valid, else ask again (restart loop)
      if (!isValid(input)) { continue; }
      
      //check input
      likeness = getLikeness(input);
      
      if (likeness == PASS_LENGTH){ //if correct password is entered
        victory = true;
        break;
      } else {
        attemptsRemaining--;
  ...
  //return likeness (assuming it was checked by isValid)
  public int getLikeness(String input){
    int result = 0;
  ...
  //return String[] of words of length n randomly picked from big list
  public String[] get10Words(){
    String[] result = new String[10];
    int randomIndex;
    
    int i = 0;
    while (i < 10) {
      randomIndex = random.nextInt(words5.length);
      if (!(Arrays.asList(result).contains(words5[randomIndex]))){
        result[i] = words5[randomIndex];
        i++;
      }
    }
    return result;
  ...
  public void printChoices(String[] array){
    for (int i = 0; i < array.length; i++){
      System.out.println(array[i]);
    }
  }
  
  //String[] of 5-letter words
  String[] words5 = {
